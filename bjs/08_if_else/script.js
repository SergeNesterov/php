...
document.getElementById('btnPlay').addEventListener('click', function () {  
    ...
    document.getElementById('btnLess').addEventListener('click', function () { 
        if (gameRun) {
            if (minValue === maxValue || minValue == answerNumber) {
                const caseRandom = Math.round(Math.random() * 4);
                switch (caseRandom) {
                    case 0:
                        responsePhrase = `Вы загадали неправильное число!\n\u{1F914}`
                        break;

                    case 1:
                        responsePhrase = `Вы забыли, какое число загадали?\n\u{1F92A}`
                        break;

                    case 2:
                        responsePhrase = `Вы ошиблись с числом!\n\u{1F9D0}`
                        break;

                    case 3:
                        responsePhrase = `Не жульничайте!\n\u{1F620}`
                        break;
                }
                answerField.innerText = responsePhrase;
                gameRun = false;
            } else {
                ...
                switch (caseRandom) {
                    case 1:
                        responsePhrase = `Наверное, это число `
                        break;

                    case 2:
                        responsePhrase = `Возможно `
                        break;

                    case 3:
                        responsePhrase = `Это число `
                        break;

                    case 4:
                        responsePhrase = `Скорее всего это число `
                        break;
                }
                answerField.innerText = answerNumber >= 0 ? numberToText().length < 20 && answerNumber >= 0 ? `Вы загадали число ${numberToText()}?` : `Вы загадали число ${answerNumber}?` : numberToText().length < 20 ? `Вы загадали число минус ${numberToText()}?` : `Вы загадали число ${answerNumber}?`;
            }
    ...
    document.getElementById('btnOver').addEventListener('click', function () {
        if (gameRun) {
            if (minValue === maxValue) {
                const caseRandom = Math.round(Math.random() * 3);
                switch (caseRandom) {
                    case 0:
                        responsePhrase = `Вы загадали неправильное число!\n\u{1F914}`
                        break;
                ...
                switch (caseRandom) {
                    case 0:
                        responsePhrase = `Вы загадали число `
                        break;
    ...
    document.getElementById('btnEqual').addEventListener('click', function () { 
        if (gameRun) {
            ...
            switch (caseRandom) {
                case 0:
                    responsePhrase = `Я всегда угадываю\n\u{1F60E}`
                    break;
                ...
                case 2:
                    responsePhrase = `Отлично!\n\u{1F973}`
                    break;

                case 3:
                    responsePhrase = `Я выиграл!\n\u{1F929}`
                    break;
            }
            answerField.innerText = responsePhrase;
            gameRun = false;
